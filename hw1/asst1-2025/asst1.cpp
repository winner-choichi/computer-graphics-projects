...
struct ShaderState
{
  ...
  // ass1 uniform append
  GLint h_uWindowSizeX, h_uWindowSizeY;
  GLint h_uTime;
  ...
  ShaderState(const char *vsfn, const char *fsfn)
  {
    ...
    // ass1 Retrieve handles to uniform variables
    h_uWindowSizeX = safe_glGetUniformLocation(h, "uWindowSizeX");
    h_uWindowSizeY = safe_glGetUniformLocation(h, "uWindowSizeY");
...
static shared_ptr<GlTexture> g_tex0; // our global texture instance
...
static void display(void)
{
  ...
  // ass1 시간값 받아와서 유니폼에 할당
  float currentTime = glutGet(GLUT_ELAPSED_TIME) / 1000.0; // ms -> s
  safe_glUniform1f(curSS.h_uTime, currentTime);
  safe_glUniform1f(curSS.h_uVertexScale, sin(currentTime * 10.0));

  safe_glUniform1i(curSS.h_uTexUnit0, 0); //0 refers to GL_TEXTURE0

  // ass1
  if (g_width < g_height)
  {
    safe_glUniform1f(curSS.h_uWindowSizeX, 1);
    safe_glUniform1f(curSS.h_uWindowSizeY, (float)g_width / (float)g_height);
  }
  else
  {
    safe_glUniform1f(curSS.h_uWindowSizeX, (float)g_height / (float)g_width);
    safe_glUniform1f(curSS.h_uWindowSizeY, 1);
  }
...
static void initTextures()
{
  ...
  loadTexture(*g_tex0, "spinning_cat_256.ppm");
...
int main(int argc, char **argv)
{
  try
  {
    ...
    glutIdleFunc(display); // ass1 display 계속 호출
