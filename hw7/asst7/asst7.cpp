...
static bool interpolateAndDisplay(float t)
{
  ...
  const int size = g_script->get_script_size();

  if (size < 4 || i + 3 >= size)
  {
    Frame fa = g_script->get_frame_at(size - 2);
    fa.paste_to_scene(g_world);
    return true;
  }
  Frame f0 = g_script->get_frame_at(i);
  Frame f1 = g_script->get_frame_at(i + 1);
  Frame f2 = g_script->get_frame_at(i + 2);
  Frame f3 = g_script->get_frame_at(i + 3);
  // Frame fa = get_frame_linear_interpolation(f0, f1, a);
  Frame fa = get_frame_CR_interpolation(f0, f1, f2, f3, a);
  fa.paste_to_scene(g_world);
  glutPostRedisplay();
  return false;
}
