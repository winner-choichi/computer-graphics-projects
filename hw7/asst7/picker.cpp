...
{
  if (dynamic_pointer_cast<SgRbtNode>(nodeStack_.back()) == NULL)
  {
    return false;
  }
  idCounter_ += 1;
  addToMap(idCounter_, dynamic_pointer_cast<SgRbtNode>(nodeStack_.back()));
  Cvec3 idColor_ = idToColor(idCounter_);
  safe_glUniform3f(drawer_.getCurSS().h_uIdColor, idColor_[0], idColor_[1], idColor_[2]);
  return drawer_.visit(node);
}
