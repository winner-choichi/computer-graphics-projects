...
void main() {
  vec3 texNormal = texture2D(uTexNormal, vTexCoord).xyz;
  vec3 normal = normalize(vNTMat * (2.0 * texNormal - 1.0));
  ...
  vec3 color = texture2D(uTexColor, vTexCoord).xyz * diffuse + specular * vec3(0.4, 0.4, 0.4);
