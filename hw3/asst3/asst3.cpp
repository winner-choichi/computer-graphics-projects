...
static shared_ptr<Geometry> g_ground, g_cube, g_shpere;
...
static void initSphere()
{
  ...
  g_shpere.reset(new Geometry(&vtx[0], &idx[0], vbLen, ibLen));
}
...
static void drawStuff()
{
  ...
  float z;
  if (ctrlObj == 's' && wldSky == 'w' && viewpoint == 's')
  {
    z = invEyeRbt.getTranslation()[2]; // world라서 O생략, t의 z성분만
    drawArc = true;
  }
  else if ((ctrlObj == 'l' || ctrlObj == 'r') && ctrlObj != viewpoint)
  {
    if (ctrlObj == 'l')
    {
      z = (invEyeRbt * g_objectRbt[0]).getTranslation()[2];
    }
    else
    {
      z = (invEyeRbt * g_objectRbt[1]).getTranslation()[2];
    }
    drawArc = true;
  }
  else
  ...
  if (drawArc)
  {
    ...
    MVM = rigTFormToMatrix(invEyeRbt * g_arcballRbt) * Matrix4::makeScale(Cvec3(arcballEyeRadius));
    NMVM = normalMatrix(MVM);
    sendModelViewNormalMatrix(curSS, MVM, NMVM);
...
static void motion(const int x, const int y)
{
  ...
  if (g_mouseClickDown)
  {
    ...
    if (ctrlObj == 's' && (viewpoint == 'l' || viewpoint == 'r'))
      m = RigTForm(); // cube가 sky 바꾸려 하면 변환 identity로 안바뀌게.
    switch (ctrlObj)
    {
    case 's':
