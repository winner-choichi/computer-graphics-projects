...
public:
  ...
  {
    double w = (*this)[0];
    Cvec3 v((*this)[1], (*this)[2], (*this)[3]);
    double v_len = norm(v);

    if (v_len < CS175_EPS)
      return *this;

    double theta = atan2(v_len, w);
    double new_theta = theta * a;

    double new_w = cos(new_theta);
    Cvec3 new_v = v * sin(new_theta) / v_len;

    return Quat(new_w, new_v);
  }
